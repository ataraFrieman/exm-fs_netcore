...
{
    public class SendFileToDB<TMatchFile, TVM> where TMatchFile : IConvertToVM<TVM>, new()
    {
        private List<TMatchFile> listFile;
        ...
        public SendFileToDB(string subUrl, string filePath)
        {
            this.subUrl = baseUrl + subUrl;
            ConvertFileToObjectList(filePath);
            FillTVMList();
            SendListToDBAsync();
        }

        private void FillTVMList()
        {
            foreach (var li in listFile)
            {
                var st = li.Convert();
                list.Add(st);
            }
        ...
        private void ConvertFileToObjectList(string path)
        {
            switch (path.Substring(path.Length - 3, 3))
            {
                case "csv":
                    listFile = ReadingCSVFiles<TMatchFile>.GetFromFileToList(path);
                    break;
            ...
            using (var request = new HttpRequestMessage(HttpMethod.Post, url))
            {
                ...
                using (var stringContent = new StringContent(json, Encoding.UTF8, "application/json"))
                {
                    ...
                    var response = await client.SendAsync(request, HttpCompletionOption.ResponseHeadersRead, CancellationToken.None).ConfigureAwait(false);
                    {
                        ...
                        {
                            var quzeResponse = JsonConvert.DeserializeObject<Response<TVM>>(resultString);
