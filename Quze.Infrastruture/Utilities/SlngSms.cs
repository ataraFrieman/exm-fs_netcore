...
namespace Quze.Infrastruture.Utilities
{
    public class SlngSMS : ISMS
    {
        ...
        const string AppointmentDetailsMessage = "שלום, \n \n {0} : נקבע לך תור אצל {1}  בתאריך  {2} בשעה {3} בסניף {4} , על מנת לראות פרטים נוספים ומה עליך להתכונן תוכל להכנס בכל עת לקישור: {5}";
        ...
        public void SendAppointmentDetails(string to, string organization, string serviceProvider, DateTime date, string branch, string URL)
        {
            var message = string.Format(AppointmentDetailsMessage, organization, serviceProvider, date.ToString("dd/MM/yyyy"), date.ToString("HH:mm"), branch, URL );
            SendMessage(to, "AppointmentDetails", message);
        }
